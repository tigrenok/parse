			    	...
			    	if(isset($_POST['UserRecoveryForm'])) {
			    		...
			    		if($form->validate()) {
							...
							$activation_url = 'http://' . $_SERVER['HTTP_HOST'].$this->createUrl(implode(Yii::app()->controller->module->recoveryUrl),array("activkey" => $user->activkey, "email" => $user->email));
                                                      ...
                                                      $email->Subject = Yii::t('user', 'Востонавление пароля');
                                                      $email->Body = $this->renderFile('protected/views/email/recovery.php', array('login'=>$user->email,'activate_url' => $activation_url), true);
                                                      $email->Send();
